...
func _ready():
	...
	# Charger les valeurs des ProgressBars depuis StatsManager
	for stat in get_children():
		var progress_bar = stat.get_node("TextureProgressBar") if stat.has_node("TextureProgressBar") else null
		if progress_bar:
			progress_bar.value = StatsManager.get_stat_value(stat.name)  # Récupère la valeur depuis l'Autoload
...
func update_sizes():
	...
	for stat in get_children():  # Parcours tous les éléments (Stat1, Stat2, etc.)
		...
		var value = StatsManager.get_stat_value(stat.name)
		progress_bar.value = value

		var texture = get_texture_for_stat(stat.name, value)
		if texture:
			progress_bar.texture_progress = texture  # <- Ce doit être ici !
